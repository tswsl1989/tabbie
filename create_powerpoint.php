...
function add_text_element($slide, $text, $width, $height, $xoffset, $yoffset, $fontsize){
...
if(isset($generate)){
	...
	*/
	...
	add_text_element($currentSlide, "Round $roundno", 480, 300, 480, 560, 80);
	...
	add_text_element($currentSlide, "The Motion", 600, 300, 340, 560, 80);
	
	// Slide with the motion on it
	$currentSlide = $presentation->createSlide();
	if($otherslides != "") add_design_element($currentSlide, $otherslides, 960, 720, 0, 0);
	add_text_element($currentSlide, get_motion_for_round($roundno), 600, 300, 170, 180, 80);
